'''
Functions on running classification modeling on a binary machine-learning problem, using logistic regression.
These should be used for train/validation purposes.
'''
...
def log_precision_and_recall_curves(X,y,C=0.95):
    '''
    Arguments: takes in a set of features (X) and labels (y) data, and a C hyperparameter to run logistic regression.
    NOTE: this is intended for train/validation purposes.
    Returns: a plot of precision and recall curves after running logistic regression classification.
    '''
...
def log_precision_recall_curve_generator(X,y,C=0.95):
    ...
    Arguments: takes in a set of features (X) and labels (y) data, and a C hyperparameter to run logistic regression.
    NOTE: this is intended for train/validation purposes.
    Returns: a plot of a precision-recall curve after running logistic regression classification.
    '''
    #Split Data, fit log regression model:
    X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=0.2, random_state=5, stratify=y)
...
def log_accuracy_scorer(X, y, threshold=0.5, C=0.95, beta=0.5):
    '''
    Arguments: takes in a set of features X and a target variable y.  Y is a classification (0/1).  Default C is 0.95, can be changed - this is the logistic regresstion hyper parameter.
    Also includes a threshold, default of 0.5, for classification purposes.  Beta is default to 0.5, which is used to calculate the Fbeta score.
    Returns: Performs logistic regression classification and returns the scoring metrics.
    '''
...
def log_roc_curve_generator(X,y,C=0.95):
    '''
    Arguments: takes in a set of features (X) and labels (y) data, and a C hyperparameter to run logistic regression.  C is set to 0.95 default.
    NOTE: this is intended for train/validation purposes.
    Returns: a plot of a ROC curve after running logistic regression classification, along with ROC AUC score.
    '''
    #Splitting into train and val sets:
    X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=0.2, random_state=5, stratify=y)
...
def log_accuracy_scorer_k_fold(X, y, k=5, threshold=0.5, C=0.95):
    ...
    Returns: Performs logistic regression classification and returns the scores.
    '''
